  ...
  <v-container class="px-4 py-12">
    ...
    <v-row justify="center" align="stretch">
        ...
        <v-card v-scroll-animate class="h-100 pa-8 rounded-xl controls-card before-enter-slide-left delay-2" elevation="0">
          ...
          <div class="mb-10">
            ...
            <p class="text-caption text-grey-darken-1 mt-3">
              ...
              Cada edificio requiere 1 Servidor Central (Raspberry Pi) y 1 Kit de cableado.
            </p>
          </div>
          ...
          <div>
            ...
            <p class="text-caption text-grey-darken-1 mt-3">
              <v-icon size="x-small" color="grey" class="mr-1">mdi-information</v-icon>
              Cada sala incluye 1 microcontrolador ESP32, 2 sensores y 2 actuadores Sonoff.
            </p>
          </div>
        </v-card>
      </v-col>
      ...
      <v-col cols="12" md="6" lg="5">
        <v-card v-scroll-animate class="h-100 pa-8 rounded-xl ticket-card before-enter-slide-right delay-3" elevation="6">
          ...
          <div class="mb-6">
            <h4 class="text-caption text-uppercase text-grey-darken-1 font-weight-black mb-3 tracking-wide">Nodos de Monitoreo</h4>
            <div class="d-flex justify-space-between mb-2">
            ...
            <div class="d-flex justify-space-between mb-2">
              <span class="text-body-2 text-grey-darken-3">{{ numSalones * 2 }}x Sonoff Basic R2</span>
              <span class="text-body-2 font-weight-bold text-grey-darken-4">${{ formatPrice(numSalones * precios.sonoff * 2) }}</span>
            </div>
          </div>
          ...
          <div class="d-flex justify-space-between align-center bg-blue-darken-3 pa-5 rounded-xl text-white elevation-3">
            <div>
              <span class="text-subtitle-2 text-blue-lighten-4 d-block mb-1 text-uppercase">Total Estimado</span>
              <span class="text-caption text-blue-lighten-2">Aprox. ${{ formatPrice(costoPromedioSalon) }} por sala</span>
            </div>
...
// Valores iniciales (Ejemplo de tu documento: 16 edificios y 128 salones)
const numEdificios = ref(16)
const numSalones = ref(128)

// Precios unitarios extraídos directamente de tu Memoria Técnica
const precios = {
  esp32: 185,
  pir: 59,
  sct: 129,
  sonoff: 150, // Se requieren 2 por salón
  raspberry: 1800,
  cableado: 600
}
...
const costoSalones = computed(() => {
  const costoPorSalon = precios.esp32 + precios.pir + precios.sct + (precios.sonoff * 2)
...
const costoPromedioSalon = computed(() => {
  return numSalones.value > 0 ? (costoTotal.value / numSalones.value).toFixed(0) : 0
})

// Formateador para poner las comitas en los miles (ej. 124,544)
...
/* =========================================
   ESTILOS DE TARJETAS (Interactive Dashboard)
========================================= */
.controls-card {
  background-color: #f8fafc !important; /* Gris muy claro azulado */
  border: 1px solid #e2e8f0 !important;
}

.ticket-card {
  background-color: #ffffff !important;
  border-top: 8px solid #1976D2 !important; /* Franja azul arriba tipo ticket */
}

/* Divisores tipo línea punteada para simular un recibo */
.border-dashed {
...
/* =========================================
    ANIMACIONES ON SCROLL (Slide Cruzado)
========================================= */

/* Para el Título (Aparece desvaneciéndose) */
.before-enter-fade {
  opacity: 0;
...
/* Para los Controles (Entran desde la izquierda) */
...
/* Para el Ticket (Entra desde la derecha) */
